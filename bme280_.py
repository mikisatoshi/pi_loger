...
class BME280():
  ...
  def get_calib_param(self):
  	...
  	for i in range (0x88,0x88+24):
  		self.calib.append(bus.read_byte_data(i2c_address,i))
  	self.calib.append(bus.read_byte_data(i2c_address,0xA1))
  	for i in range (0xE1,0xE1+7):
  		self.calib.append(bus.read_byte_data(i2c_address,i))

  	self.digT.append((self.self.calib[1] << 8) | self.calib[0])
  ...
  def readData(self):
    ...
    __ = compensate_T(self.temp_raw)
    __ = compensate_P(self.pres_raw)
    __ = compensate_H(self.hum_raw)
  ...
  def getData(self):
    ...
    temp = compensate_T(self.temp_raw)
    pres = compensate_P(self.pres_raw)
    humi = compensate_H(self.hum_raw)
